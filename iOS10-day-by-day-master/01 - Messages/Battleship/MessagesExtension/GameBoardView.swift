...
extension GameBoardView {
    ...
    func reset() {
        CellStyle = Array(repeating: .deselected, count: 9)
        reloadData()
        layoutIfNeeded()
    }
    ...
    var selectedCells: [Int] {
        ...
        for (index, selected) in CellStyle.enumerated() where selected == .selectedGreen || selected == .selectedRed {
            cells.append(index)
        }
    ...
    func toggleCellStyle(at cellIndex: Int) {
        let style: CellStyle = CellStyle[cellIndex] == .selectedGreen ? .deselected : .selectedGreen
    ...
    func alterCell(at index: Int, applying selectionStyle: CellStyle) {
        CellStyle[index] = selectionStyle
...
extension GameBoardView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        delegate(cell, for: cellStyles[indexPath.row])
